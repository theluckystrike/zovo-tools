...
Scans each tool's index.html for <title> and <meta name="description">.
Categorizes tools by keyword matching and generates a complete index page.
"""
...
CATEGORY_KEYWORDS = {
    "Developer": [
        "json", "xml", "css-grid", "css-flex", "css-clip", "css-filter", "css-gradient",
        "js", "javascript", "html-", "api", "regex", "code", "sql", "git", "docker",
        "curl", "yaml", "base64", "jwt", "uuid", "url-encoder", "url-decode", "bcrypt",
        "hash", "cors", "csp", "sitemap", "robots", "schema-", "dns", "ip-lookup",
        "http", "ascii", "binary-to", "hex-to", "hex-color", "chmod", "cron", "diff",
        "favicon", "webhook", "minif", "formatter", "beautif", "svg", "dom-",
        "markdown", "csv-to-json", "json-to", "xml-to", "csv-to", "json-path",
        "graphql", "npm", "webpack", "typescript", "python", "upc-code",
        "barcode-gen", "open-graph", "meta-tag-gen", "htaccess", "toml",
        "color-code", "responsive-test", "user-agent", "lorem-ipsum",
        "syntax", "html-entity", "html-table", "html-form", "html-email",
        "css-animation", "css-transform", "text-diff", "code-",
        "qr-code", "pixel-ruler", "screen-resolution",
        "url-shortener", "url-encoder-decoder", "web-scraper",
    ],
    "Finance": [
        "calculator" if False else None,  # placeholder, handled specially
        "loan", "mortgage", "tax", "interest", "savings", "investment", "budget",
        "debt", "payroll", "salary", "401k", "apy", "cagr", "amortization", "bond",
        "break-even", "payment", "refinance", "equity", "emi", "roi",
        "compound-interest", "simple-interest", "inflation", "retirement",
        "annuity", "depreciation", "margin-calculator", "markup-calculator",
        "discount-calculator", "tip-calculator", "tip-split", "percentage",
        "profit", "currency", "exchange", "paycheck", "take-home", "hourly",
        "overtime", "net-worth", "income", "expense", "cost-of-living",
        "rent", "lease", "car-", "auto-", "fuel", "gas-", "mpg",
        "down-payment", "closing-cost", "home-afford", "stamp-duty",
        "capital-gains", "estate-tax", "inheritance",
        "unit-price", "price-", "wage", "pay-raise", "timecard",
        "time-to-decimal", "time-calculator", "time-card",
        "finance", "financial", "money", "billing", "cash",
        "adp-payroll", "payroll-calculator", "pay-stub",
        "stock", "dividend", "credit-card", "apr",
        "sales-tax", "vat", "gst", "tax-bracket", "tax-refund",
        "age-calculator", "date-calculator", "days-between",
        "voltage-divider", "wire-gauge", "ohms-law", "resistor",
        "concrete-calculator", "fence-calculator", "gravel-calculator",
        "square-footage", "mulch-calculator", "soil-calculator",
        "btu-calculator", "board-foot", "deck-", "drywall", "paint-",
        "roofing", "tile-", "wallpaper", "pool-volume",
        "volume-calculator", "area-calculator", "water-intake",
        "usd-to", "eur-to", "gbp-to", "inr-to", "btc-to",
    ],
    "Design": [
        "color-p", "color-wheel", "color-mixer", "color-blind",
        "gradient", "shadow", "border-radius", "grid-gen", "flexbox",
        "animation-gen", "font-pair", "svg-edit", "image-resize", "photo",
        "pixel", "palette", "contrast-check", "glassmorphism", "clip-path",
    ...
    "Writing": [
        "grammar", "readability", "word-counter", "paraphrase", "headline",
        "citation", "apa-", "bibliography", "essay", "resume", "cover-letter",
        "text-case", "text-repeat", "text-summar", "text-to-speech",
        "text-to-hand", "text-gen", "word-cloud", "acrostic",
        "ai-detector", "ai-humanizer", "article", "blog",
        "poem", "story", "haiku", "rhyme", "synonym", "antonym",
        "thesaurus", "proofread", "plagiarism", "rewrite",
        "tone-", "sentiment", "passive-voice", "writing",
        "letter-", "email-writer", "bio-gen", "slogan",
        "character-count", "sentence", "paragraph",
        "name-generator",
    ],
    "Education": [
        "gpa", "grade-calc", "typing-speed", "typing-test",
        "chemistry", "periodic-table", "fibonacci", "binary-search",
        "graphing", "equation", "fraction", "algebra", "geometry",
        "statistics", "probability", "matrix", "quadratic",
        "scientific-calc", "math", "flash-card", "flashcard",
        "study", "quiz", "trivia", "word-problem", "word-scramble",
        "anagram", "number-base", "roman-numeral",
        "multiplication", "division", "prime-number",
    ...
    "Media": [
        ...
        "image-compressor", "speech-to-text",
        "screen-recorder", "ebook", "podcast",
        "youtube", "spotify", "music", "sound",
        "tuner", "metronome", "bpm", "drum",
        "ringtone", "wav-to", "flac", "ogg",
        "subtitle", "srt", "vtt", "karaoke",
    ...
    "Productivity": [
        ...
        "timezone", "utc-to", "world-clock",
        "stopwatch", "alarm", "reminder",
        "bookmark", "clipboard", "vcard",
        "ics-calendar", "unit-converter",
    ...
    "Health": [
        ...
        "vision", "hearing", "allergy", "symptom",
        "running-pace", "pace-calc", "vo2", "pushup",
    ],
    "Utility": [],  # fallback
}
...
# Newly added tools — the most recently modified ones will be picked in code below
# We'll pick 8 from those not in the original 434

def extract_title_and_desc(filepath):
    """Extract <title> and <meta name='description'> from an HTML file."""
    try:
        with open(filepath, "r", encoding="utf-8", errors="replace") as f:
            content = f.read(20000)  # read first 20K chars, enough for head
    except Exception:
        return None, None
...
def categorize_tool(slug, title, desc):
    """Categorize a tool based on its slug, title, and description."""
    text = f"{slug} {(title or '').lower()} {(desc or '').lower()}"
    ...
    # Special: calculators that aren't caught above go to Finance
    if "calculator" in slug or "calculator" in text:
        return "Finance"

    # Special: converter goes to Utility
...
def scan_tools():
    """Scan all tool directories and return tool data."""
    tools = []
    for entry in sorted(os.listdir(TOOLS_DIR)):
        dirpath = os.path.join(TOOLS_DIR, entry)
        if not os.path.isdir(dirpath):
            continue
        if entry in ("articles", ".git", ".github", "node_modules", "__pycache__", "scripts", "assets", "images", "css", "js"):
            continue
        if entry.startswith("."):
            continue
        ...
        # Clean title: remove " | Zovo", " | zovo.one", " - Zovo", etc.
...
def make_card_html(tool, is_featured=False):
    ...
    return (
        f'<a href="/free-tools/{slug}/" class="card" data-cat="{cat}">'
        f'<div class="category" style="color:{color}">{cat}</div>'
        f'<h2>{display_title}</h2>'
        f'<p>{desc}</p>'
        f'<span class="tag">Free</span></a>'
    )
...
def build_html(tools):
    ...
    filter_buttons = f'<button class="filter-btn active" data-filter="all">All ({total})</button>\n'
...
<title>{total}+ Free Online Tools - Calculators, Generators & Converters | Zovo</title>
<meta name="description" content="{total}+ free online tools by Zovo. Calculators, generators, converters and developer tools. No sign-up, no ads, no tracking.">
...
<meta property="og:title" content="{total}+ Free Online Tools | Zovo">
...
<meta name="twitter:title" content="{total}+ Free Online Tools | Zovo">
...
{{
  ...
  "description": "{total}+ free online tools. Calculators, generators, converters and developer tools.",
...
<div class="hero">
<div class="hero-count"><span>{total}+</span></div>
...
<input type="text" class="search" id="search" placeholder="Search {total}+ tools..." autocomplete="off">
</div>
<div class="hero-stats">
<div class="hero-stat"><div class="num">{total}+</div><div class="label">Tools</div></div>
<div class="hero-stat"><div class="num">{num_categories}</div><div class="label">Categories</div></div>
...
<h2>All Tools</h2>
<span class="badge" style="background:rgba(255,255,255,0.06);color:#888">{total} tools</span>
</div>
